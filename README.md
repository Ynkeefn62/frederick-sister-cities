...
## Form submissions (membership, contact, newsletter signup)

The three forms on the site open the visitor's email app with a pre-filled
message (a `mailto:` link) addressed to the relevant FSCA inbox
(`treasurer@`, `info@`, or `newsletter@fredericksistercities.org`). The
visitor still has to hit send themselves — there's no backend or form
service behind this, so nothing to configure or maintain.

## Gallery timeline

The Gallery page is a clickable year timeline (`#timelineNav` /
`.year-panel` in `index.html`). To add a new year, add a dot button with
a `data-year="YYYY"` and a matching `<div class="year-panel"
data-year-panel="YYYY">` block with `.event-card` entries inside — copy
an existing year's markup as a template. The leftmost dot is shown by
default, so add new years at the front of the list.
...
- Optionally connect EmailJS (see "Form submissions" above) so forms
  submit silently instead of opening the visitor's email app.
- Add a real payment processor if you want membership dues or donations
  collected on the site rather than mailed in.
